...
<div class="@if ($mt_2) mt-2 @endif w-full">
    <div class="md-textbox relative border-b-2 border-[#305582] w-full">
        <input oninput="handleInputChange('{{ $name }}')"
            @if ($type != 'password') value="{{ $value ?? old($name) }}" @endif id="{{ $name }}"
            type="{{ $type }}" name="{{ $name }}" autocomplete="{{ $autocomplete }}"
            @if ($readonly === true) readonly @endif class="h-14 w-full pl-4 border-none focus:ring-0 pr-10 bg-transparent" />
