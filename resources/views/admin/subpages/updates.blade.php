...
@section('title')
    Rádio ostrov | Novinky
@endsection

@section('content')
    <section class="">
        <x-admin-section-header>Novinky</x-admin-section-header>
        <div class="flex flex-col items-center my-8">
            <div class="flex flex-col items-center grow gap-4 w-[80%]">
                <div
                    ...
                    <h3 class="text-lg font-semibold mb-2">Pridať novinku</h3>
                    <form method="post" action="{{ route('admin.addUpdate') }}" class="w-full flex gap-4 mt-4">
                        @csrf
                        <div class="flex flex-col flex-1">
                            <label for="text" class="w-full flex items-center">Text novinky <span
                                    class="text-red-500">*</span></label>
                            <input class="" type="text" name="text" id="text" maxlength="255"
                                placeholder="Text novinky" required>
                            @if ($errors->has('text'))
                                <p class="text-red-500 text-sm mt-1">{{ $errors->first('text') }}</p>
                            @endif
                        </div>
                        <div class="flex flex-col flex-1">
                            <label for="endDate" class="w-full flex items-center">Dátum konca novinky <span
                                    class="text-red-500"></span></label>
                        ...
                        <div class="flex flex-col flex-1">
                            <label for="action" class="w-full flex items-center">Akcia (link)</label>
                            <input class="flex-1" type="text" name="action" id="action"
                                placeholder="Akcia (voliteľné)">
                ...
                @foreach ($updates as $update)
                    <div
                        ...
                        <p class="text-gray-600">Do: {{ $update->end_date }}</p>
                        <form action="{{ route('admin.deleteUpdate') }}" method="post"
                            class="flex items-center justify-center w-full">
                            @csrf
                            <input type="hidden" name="updateId" value="{{ $update->id }}">
                            <button type="submit" title="Zmazať novinku" class="text-red-400 hover:text-red-500">
