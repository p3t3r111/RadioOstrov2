...
@section('content')
    <section class="py-8 flex flex-col items-center gap-6">
        <h2 class="text-center text-base lg:text-3xl font-bold">{{ __('vote.active.title') }} | {{ __('vote.days.' . $den) }}
            {{ $datum }}</h2>
        <div class="container">
            <form action="{{ route('vote.vote') }}" method="post"
                ...
                <a href="{{ route('profile.show') }}#personal-favorite-songs"
                    class="text-center text-sm px-2 lg:text-base hover:underline hover:text-ostrov">
                ...
                <p>{{ __('vote.active.remaining_votes') }}: <span
                        id="remainingVotes">{{ $maxVotes - $voteCounterUserTotal }}</span> |
                    {{ __('vote.active.vote_weight') }}: {{ $voteWeight }}</p>
                ...
                <button type="submit" id="confirmButton"
                    class="mt-3 p-2 text-black text-center bg-primaryAction text-base rounded-lg lg:text-xl">{{ __('vote.active.submit') }}</button>
            </form>
        </div>
    </section>
